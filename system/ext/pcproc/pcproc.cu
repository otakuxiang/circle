...
__global__ void compute_sdf_kernel(
    const PCAccessor input_pc,
    const PCAccessor gt_normal,
    const PCAccessor gt_pc,
    const float* __restrict__ input_nn_dist, 
    const int* __restrict__ input_nn_ind,
    const float voxel_size,const float radius,
    PCAccessor output_sdf
){
    const uint pc_id = blockIdx.x * blockDim.x + threadIdx.x;
    if (pc_id >= input_pc.size(0)) {
        return;
    ...
    const float cur_dist = input_nn_dist[pc_id];
    const int cur_ind = input_nn_ind[pc_id];
    if (cur_ind >= gt_pc.size(0)) {
        printf("error!");
        return;   
    } 
    if(cur_dist > radius){
        output_sdf[pc_id][0] = NAN;
        return;
    } 
    float3 point = make_float3(input_pc[pc_id][0],input_pc[pc_id][1],input_pc[pc_id][2]);
    float3 c_point = make_float3(gt_pc[cur_ind][0],gt_pc[cur_ind][1],gt_pc[cur_ind][2]);
    float3 c_normal = make_float3(gt_normal[cur_ind][0],gt_normal[cur_ind][1],gt_normal[cur_ind][2]);
    float sdf;
    if(cur_dist < 0.02){
        sdf = dot((point - c_point),c_normal) ;
    }
    else{
        float sign = dot((point - c_point),c_normal);\
        sdf = sqrt(dot((point - c_point),(point - c_point)));
        if (sign < 0){
            sdf = -sdf;
        }
    }
    output_sdf[pc_id][0] = sdf / voxel_size;
}
...
torch::Tensor compute_sdf(torch::Tensor input_pc, torch::Tensor gt_pc, torch::Tensor gt_normal
    ,float voxel_size,float radius) {
    CHECK_INPUT(input_pc);
    CHECK_INPUT(gt_pc);
    CHECK_INPUT(gt_normal);
    size_t gt_point = gt_pc.size(0);
    size_t n_point = input_pc.size(0);

    tinyflann::KDTreeCuda3dIndex<tinyflann::CudaL2> knn_index(gt_pc.data_ptr<float>(), gt_point);
    ORcudaKernelCheck
    knn_index.buildIndex();
    thrust::device_vector<float> dist(n_point);
    thrust::device_vector<int> indices(n_point);
    knn_index.knnSearch(input_pc.data_ptr<float>(), n_point, 4, (int*) indices.data().get(),
                        (float*) dist.data().get(), 1);
    ORcudaKernelCheck
    torch::Tensor output_sdf = torch::empty({(long) n_point,1}, torch::dtype(torch::kFloat32).device(torch::kCUDA));

    dim3 dimBlock = dim3(128);
    dim3 dimGrid = dim3(div_up(n_point, dimBlock.x));
    compute_sdf_kernel<<<dimGrid, dimBlock>>>(
            input_pc.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
            gt_normal.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
            gt_pc.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
            dist.data().get(), indices.data().get(),voxel_size,radius,
            output_sdf.packed_accessor32<float, 2, torch::RestrictPtrTraits>());
    ORcudaKernelCheck
    // printf("ok\n");
    return output_sdf;
}

__global__ void valid_mask_kernel(const PCAccessor input_pc, const float* __restrict__ input_nn_dist, float radius,
                                             MaskAccessor output_mask) {
    const uint pc_id = blockIdx.x * blockDim.x + threadIdx.x;
    if (pc_id >= input_pc.size(0)) {
        return;
    }
    output_mask[pc_id] = input_nn_dist[pc_id] < radius*radius;
}


torch::Tensor valid_mask(torch::Tensor input_pc, torch::Tensor gt_pc,float radius) {
    CHECK_INPUT(input_pc);
    CHECK_INPUT(gt_pc);
    size_t gt_point = gt_pc.size(0);
    size_t n_point = input_pc.size(0);
    ...
    valid_mask_kernel<<<dimGrid, dimBlock>>>(
            input_pc.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
            dist.data().get(), radius,
            output_mask.packed_accessor32<bool, 1, torch::RestrictPtrTraits>());
    // printf("ok\n");
    return output_mask;
}