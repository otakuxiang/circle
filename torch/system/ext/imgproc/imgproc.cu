...
__global__ static void compute_normal_weight_kernel(const PCMapAccessor pc_map, PCMapAccessor normal_weight,
    ...
    // Compute Uncertainty:
    // https://www.wolframalpha.com/input/?i=plot+0.0019*%28x-0.4%29*%28x-0.4%29+%2B+0.0001+%2F+sqrt%28x%29+*+%28acos%28y%29%2F%28pi%2F2-acos%28y%29%29%29%5E2%3B+y+from+-1+to+1%2Cx+from+0.5+to+5.0
    // Generally, the farther the plane, the more uncertain we become. (~40cm is best)
    // Also, the more parallel the plane is relative to the camera plane, the more certain we become.
