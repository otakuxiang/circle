...
__global__ static void groupby_sum_kernel_backward(const ValueAccessor grad_out, const IndexAccessor indices, ValueAccessor grad_in) {
    const uint i = blockIdx.x;
    const uint l = threadIdx.x;

    float *value = &(grad_in[i][l]);
    long index = indices[i];

    float* rptr = grad_out[index].data() + l;
    *value = *rptr;
}
...
//               __FILE__, __FILE__, cudaGetErrorString(cudaPeekAtLastError()) );
//        exit(-1);
//    }
...
torch::Tensor groupby_max(torch::Tensor values, torch::Tensor indices, uint C) {
    ...
    torch::Tensor reduced_values = torch::full({C, L}, -INFINITY, torch::dtype(torch::kFloat32).device(torch::kCUDA));

    dim3 dimBlock = dim3(L);
    dim3 dimGrid = dim3(N);
    groupby_max_kernel<<<dimGrid, dimBlock>>>(
        values.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        indices.packed_accessor32<long, 1, torch::RestrictPtrTraits>(),
        reduced_values.packed_accessor32<float, 2, torch::RestrictPtrTraits>()
    );
...
torch::Tensor groupby_sum_backward(torch::Tensor grad_out, torch::Tensor grad_in,torch::Tensor sample_indexer) {
    ...
    CHECK_INPUT(grad_in);
    CHECK_INPUT(sample_indexer);
    const uint N = grad_in.size(0);
    const uint L = grad_in.size(1);

    dim3 dimBlock = dim3(L);
    dim3 dimGrid = dim3(N);
    groupby_sum_kernel_backward<<<dimGrid, dimBlock>>>(
        grad_out.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        sample_indexer.packed_accessor32<long, 1, torch::RestrictPtrTraits>(),
        grad_in.packed_accessor32<float, 2, torch::RestrictPtrTraits>()
    );
